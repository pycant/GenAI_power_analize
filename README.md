...
在给定的硬件资源和计算预算下，最小化推理延迟（latency）、降低能耗（energy/power）与最大化模型输出质量（accuracy/quality） 这三个目标无法同时达到全局最优。围绕生成式大语言模型的质效比，构建一套模型评价体系，在算力不变（相同硬件设备条件下），同时保证满足推理质量与效率的要求条件下找到能耗最低的大语言模型，换而言之就是构建一个全面、客观的GenAI（生成式人工智能）模型能效评级体系，通过多维效质比评估方法，为学术界和工业界提供标准化的模型评估工具，促进AI技术的可持续发展。
