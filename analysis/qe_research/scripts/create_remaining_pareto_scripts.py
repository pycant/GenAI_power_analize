...
基于code和math任务的模板，自动生成qa, summary, creative, translation的分析脚本
"""
...
TASKS_CONFIG = {
    'qa': {
        'description': '问答任务',
        'quality_metric_desc': '综合质量得分',
        'quality_column_index': 9,  # 需要根据实际CSV调整
        'quality_display': '综合得分'
    },
    'summary': {
        'description': '摘要任务',
        'quality_metric_desc': 'ROUGE-L得分',
        'quality_column_index': 9,
        'quality_display': 'ROUGE-L'
    },
    'creative': {
        'description': '创意写作任务',
        'quality_metric_desc': 'Distinct-2多样性',
        'quality_column_index': 9,
        'quality_display': 'Distinct-2'
    },
    'translation': {
        'description': '翻译任务',
        'quality_metric_desc': 'BLEU得分',
        'quality_column_index': 9,
        'quality_display': 'BLEU'
    }
}
...
# 数据路径
QUALITY_FILE = project_root / 'data' / 'analize' / 'results' / '{task}_quality' / '{task}_quality_summary.csv'
...
# 输出目录
OUTPUT_DIR = project_root / 'analysis' / 'qe_research' / 'results' / 'pareto_analysis' / '{task}'
...
def generate_report(df, results):
    """生成分析报告"""
    report_file = OUTPUT_DIR / '{task_upper}_PARETO_ANALYSIS_REPORT.md'
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# {description}帕累托前沿分析报告\\n\\n")
        f.write(f"**生成时间**: {{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}}\\n\\n")
        f.write("---\\n\\n")
        
        f.write("## 1. 数据概览\\n\\n")
        f.write(f"- **任务类型**: {description}（{task}）\\n")
        f.write(f"- **模型数量**: {{len(df)}}\\n")
        f.write(f"- **质量指标**: {quality_metric_desc}\\n")
        ...
        if knee_model:
            ...
            f.write(f"- **{quality_display}**: {{knee_row['quality']:.3f}}\\n")
        ...
        f.write(f"- **{quality_display}**: {{best_quality['quality']:.3f}}\\n")
        ...
        f.write(f"- **{quality_display}**: {{best_energy['quality']:.3f}}\\n")
        ...
        f.write(f"- **{quality_display}**: {{best_speed['quality']:.3f}}\\n")
        f.write(f"- **每token能耗**: {{best_speed['energy']:.3f}} J/token\\n")
        f.write(f"- **生成速度**: {{best_speed['speed']:.2f}} tokens/s\\n\\n")
        
        f.write("## 5. 完整数据表\\n\\n")
        f.write("| 模型 | {quality_display} | 能耗(J/token) | 速度(tokens/s) | 质量-能耗前沿 | 质量-速度前沿 | 3D前沿 |\\n")
        f.write("|------|--------|--------------|---------------|-------------|-------------|--------|\\n")
        ...
        f.write("\\n---\\n\\n")
        f.write("**生成脚本**: `analysis/qe_research/scripts/pareto_analysis_{task}.py`\\n")
...
def main():
    ...
    plot_pareto_2d(df, pareto_qe, 'quality', 'energy',
                   '{description}：质量-能耗帕累托前沿',
                   'pareto_quality_energy.png',
                   '{quality_display}', '每token能耗 (J/token)',
                   x_minimize=False, y_minimize=True)
    
    plot_pareto_2d(df, pareto_qs, 'quality', 'speed',
                   '{description}：质量-速度帕累托前沿',
                   'pareto_quality_speed.png',
                   '{quality_display}', 'Token生成速度 (tokens/s)',
                   x_minimize=False, y_minimize=False)
    ...
    print(f"- {task_upper}_PARETO_ANALYSIS_REPORT.md: 完整分析报告")
...
def create_script(task, config):
    ...
    script_content = SCRIPT_TEMPLATE.format(
        ...
        quality_metric_desc=config['quality_metric_desc'],
        quality_column_index=config['quality_column_index'],
        quality_display=config['quality_display']
    )
...
def main():
    """主函数"""
    print("="*80)
    print("为剩余任务创建帕累托分析脚本")
    print("="*80)
    print()
    
    for task, config in TASKS_CONFIG.items():
        create_script(task, config)
    ...
    print("="*80)
    print()
    print("下一步：运行各个任务的分析脚本")
    for task in TASKS_CONFIG.keys():
        print(f"  python analysis/qe_research/scripts/pareto_analysis_{task}.py")
