...
# 数据路径
QUALITY_FILE = project_root / 'data' / 'analize' / 'results' / 'creative_quality' / 'creative_quality_summary.csv'
...
# 输出目录
OUTPUT_DIR = project_root / 'analysis' / 'qe_research' / 'results' / 'pareto_analysis' / 'creative'
...
def generate_report(df, results):
    """生成分析报告"""
    report_file = OUTPUT_DIR / 'CREATIVE_PARETO_ANALYSIS_REPORT.md'
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 创意写作任务帕累托前沿分析报告\n\n")
        f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("---\n\n")
        
        f.write("## 1. 数据概览\n\n")
        f.write(f"- **任务类型**: 创意写作任务（creative）\n")
        f.write(f"- **模型数量**: {len(df)}\n")
        f.write(f"- **质量指标**: Distinct-2多样性\n")
        ...
        if knee_model:
            ...
            f.write(f"- **Distinct-2**: {knee_row['quality']:.3f}\n")
        ...
        f.write(f"- **Distinct-2**: {best_quality['quality']:.3f}\n")
        ...
        f.write(f"- **Distinct-2**: {best_energy['quality']:.3f}\n")
        ...
        f.write(f"- **Distinct-2**: {best_speed['quality']:.3f}\n")
        f.write(f"- **每token能耗**: {best_speed['energy']:.3f} J/token\n")
        f.write(f"- **生成速度**: {best_speed['speed']:.2f} tokens/s\n\n")
        
        f.write("## 5. 完整数据表\n\n")
        f.write("| 模型 | Distinct-2 | 能耗(J/token) | 速度(tokens/s) | 质量-能耗前沿 | 质量-速度前沿 | 3D前沿 |\n")
        f.write("|------|--------|--------------|---------------|-------------|-------------|--------|\n")
        ...
        f.write("\n---\n\n")
        f.write("**生成脚本**: `analysis/qe_research/scripts/pareto_analysis_creative.py`\n")
...
def main():
    ...
    print(f"- CREATIVE_PARETO_ANALYSIS_REPORT.md: 完整分析报告")
