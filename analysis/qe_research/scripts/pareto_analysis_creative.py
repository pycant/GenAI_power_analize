...
def load_and_prepare_data():
    ...
    quality_data = pd.DataFrame({
        ...
        'quality': quality_df['distinct_2_mean']
    })
    print(f"✓ 质量数据: {len(quality_data)} 个模型")
    
    # 加载能耗和速度数据
    energy_dict, speed_dict = load_energy_speed_data('creative', DATA_PATHS['energy'], DATA_PATHS['speed'])
...
def main():
    ...
    task_config = {
        'task_name': 'creative',
        'task_name_cn': '创意写作',
        'quality_metric': 'Distinct-2多样性',
        'report_filename': 'CREATIVE_PARETO_ANALYSIS_REPORT.md'
    }
    ...
    # 打印分析摘要
    print_analysis_summary(df, results, "创意写作任务")
