...
基于以下数据：
1. 质量指标：数值匹配准确率（numerical_match）
...
# 数据路径
QUALITY_FILE = project_root / 'data' / 'analize' / 'results' / 'math_quality' / 'math_quality_summary.csv'
...
# 输出目录
OUTPUT_DIR = project_root / 'analysis' / 'qe_research' / 'results' / 'pareto_analysis' / 'math'
...
def load_and_prepare_data():
    ...
    quality_df.columns = quality_df.iloc[0]  # 使用第一行作为列名
    quality_df = quality_df[1:].reset_index(drop=True)  # 删除第一行
    
    # 找到numerical_match列（第9列是mean值）
    # 列顺序：extraction_confidence(4列), has_answer(4列), exact_match(4列), numerical_match(4列)...
    quality_data = pd.DataFrame({
        'model': quality_df['model'],
        'quality': pd.to_numeric(quality_df.iloc[:, 9])  # numerical_match的mean列
    })
    print(f"✓ 质量数据: {len(quality_data)} 个模型")
    
    # 2. 加载能耗数据（转置）
    energy_df = pd.read_csv(ENERGY_FILE, index_col=0)
    energy_math = energy_df.loc['math'].to_dict()
    print(f"✓ 能耗数据: {len(energy_math)} 个模型")
    
    # 3. 加载速度数据（转置）
    speed_df = pd.read_csv(SPEED_FILE, index_col=0)
    speed_math = speed_df.loc['math'].to_dict()
    print(f"✓ 速度数据: {len(speed_math)} 个模型")
    ...
    for _, row in quality_data.iterrows():
        ...
        if model_full and model_full in energy_math and model_full in speed_math:
            data.append({
                'model': model_short,
                'model_full': model_full,
                'quality': row['quality'],  # 数值匹配准确率
                'energy': energy_math[model_full],  # 每token能耗（J/token）
                'speed': speed_math[model_full]  # token速度（tokens/s）
            })
...
def generate_report(df, results):
    """生成分析报告"""
    report_file = OUTPUT_DIR / 'MATH_PARETO_ANALYSIS_REPORT.md'
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 数学推理任务帕累托前沿分析报告\n\n")
        f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("---\n\n")
        
        # 1. 数据概览
        f.write("## 1. 数据概览\n\n")
        f.write(f"- **任务类型**: 数学推理（math）\n")
        f.write(f"- **模型数量**: {len(df)}\n")
        f.write(f"- **质量指标**: 数值匹配准确率（numerical_match）\n")
        ...
        for model in df[pareto_qe]['model']:
            ...
            f.write(f"- {model}: 质量={row['quality']:.1%}, 能耗={row['energy']:.3f} J/token\n")
        f.write("\n")
        ...
        for model in df[pareto_qs]['model']:
            row = df[df['model'] == model].iloc[0]
            f.write(f"- {model}: 质量={row['quality']:.1%}, 速度={row['speed']:.2f} tokens/s\n")
        f.write("\n")
        ...
        for model in df[pareto_3d]['model']:
            row = df[df['model'] == model].iloc[0]
            f.write(f"- {model}: 质量={row['quality']:.1%}, 能耗={row['energy']:.3f} J/token, 速度={row['speed']:.2f} tokens/s\n")
        f.write("\n")
        ...
        if knee_model:
            ...
            f.write(f"- **数值匹配准确率**: {knee_row['quality']:.1%}\n")
        ...
        f.write(f"- **数值匹配准确率**: {best_quality['quality']:.1%}\n")
        ...
        f.write(f"- **数值匹配准确率**: {best_energy['quality']:.1%}\n")
        ...
        f.write(f"- **数值匹配准确率**: {best_speed['quality']:.1%}\n")
        f.write(f"- **每token能耗**: {best_speed['energy']:.3f} J/token\n")
        f.write(f"- **生成速度**: {best_speed['speed']:.2f} tokens/s\n\n")
        
        # 5. 数据表
        f.write("## 5. 完整数据表\n\n")
        f.write("| 模型 | 准确率 | 能耗(J/token) | 速度(tokens/s) | 质量-能耗前沿 | 质量-速度前沿 | 3D前沿 |\n")
        f.write("|------|--------|--------------|---------------|-------------|-------------|--------|\n")
        ...
        f.write("\n---\n\n")
        f.write("**生成脚本**: `analysis/qe_research/scripts/pareto_analysis_math.py`\n")
...
def main():
    ...
    print(f"- MATH_PARETO_ANALYSIS_REPORT.md: 完整分析报告")
