...
# 数据路径
QUALITY_FILE = project_root / 'data' / 'analize' / 'results' / 'summary_quality' / 'summary_quality_summary.csv'
...
def generate_report(df, results):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# 摘要任务帕累托前沿分析报告\n\n")
        f.write(f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("---\n\n")
        
        f.write("## 1. 数据概览\n\n")
        f.write(f"- **任务类型**: 摘要任务（summary）\n")
        f.write(f"- **模型数量**: {len(df)}\n")
        f.write(f"- **质量指标**: ROUGE-L得分\n")
        ...
        if knee_model:
            ...
            f.write(f"- **ROUGE-L**: {knee_row['quality']:.3f}\n")
        ...
        f.write(f"- **ROUGE-L**: {best_quality['quality']:.3f}\n")
        ...
        f.write(f"- **ROUGE-L**: {best_energy['quality']:.3f}\n")
        ...
        f.write(f"- **ROUGE-L**: {best_speed['quality']:.3f}\n")
        f.write(f"- **每token能耗**: {best_speed['energy']:.3f} J/token\n")
        f.write(f"- **生成速度**: {best_speed['speed']:.2f} tokens/s\n\n")
        
        f.write("## 5. 完整数据表\n\n")
        f.write("| 模型 | ROUGE-L | 能耗(J/token) | 速度(tokens/s) | 质量-能耗前沿 | 质量-速度前沿 | 3D前沿 |\n")
        f.write("|------|--------|--------------|---------------|-------------|-------------|--------|\n")
        ...
        f.write("\n---\n\n")
        f.write("**生成脚本**: `analysis/qe_research/scripts/pareto_analysis_summary.py`\n")
...
def main():
    ...
    print(f"- SUMMARY_PARETO_ANALYSIS_REPORT.md: 完整分析报告")
