...
QUALITY_FILE = PROJECT_ROOT / 'data' / 'analize' / 'results' / 'translation_quality' / 'translation_quality_processed.csv'
...
def load_and_prepare_data():
    ...
    energy_dict, speed_dict = load_energy_speed_data('translation', DATA_PATHS['energy'], DATA_PATHS['speed'])
...
def main():
    ...
    # 找出最稳定的模型
    if robustness_qe['stability_scores']:
        most_stable = max(robustness_qe['stability_scores'].items(), key=lambda x: x[1])
        print(f"  最稳定模型: {most_stable[0]}")
    ...
    task_config = {
        ...
        'task_name_cn': '翻译',
        'quality_metric': 'BLEU-1得分',
        'report_filename': 'TRANSLATION_PARETO_ANALYSIS_REPORT.md'
    }
    ...
    # 打印分析摘要
    print_analysis_summary(df, results, "翻译任务")
