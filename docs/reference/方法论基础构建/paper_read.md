...
### 摘要
本系统综述调查了2018年至2024年间发表的49项关于大语言模型驱动的自动评估（LLMPAA）的同行评审研究。遵循PRISMA指南，研究从Web of Science、Scopus、IEEE、ACM数字图书馆和PubMed数据库中选取。分析显示，LLMPAA已广泛应用于阅读理解、语言教育和计算机科学领域，主要采用论文和简答题形式。虽然GPT-4和微调后的BERT等模型通常与人类评分者表现出高度一致性（例如，二次加权Kappa系数QWK=0.99，相关系数r=0.95），但其他研究报告的一致性较低（例如，组内相关系数ICC=0.45，r=0.38）。LLMPAA具有效率高、可扩展性强和个性化反馈等优势。然而，仍存在重大挑战，包括偏差、不一致性、幻觉、解释性有限、数据集质量以及隐私问题。这些发现表明，尽管LLMPAA技术具有潜力，但其有效性因情境而异。人工监督对于确保公平可靠的评估结果至关重要。
...
### 引言
生成式人工智能的快速发展，尤其是在医疗保健领域，需要一套统一的聊天机器人评估指标。当前的指标存在两个主要差距：缺乏统一的定义以及忽视了以用户为中心的方面。本文将探讨现有指标，引入新的以用户为中心的指标，提出一个评估框架，并讨论相关挑战。
### 大语言模型现有评估指标综述
语言模型的评估有内在和外在两种方法。内在指标计算简单，但缺乏语义和上下文理解能力，因此不足以用于医疗聊天机器人。外在指标纳入了用户视角，但现有的外在指标并未完全涵盖医疗聊天机器人的评估。一些研究将内在和外在测量方法结合起来，但仍然缺乏以用户为中心的全面指标。
### 评估医疗保健聊天机器人的关键指标
提出了一套全面的、以用户为中心的指标，用于评估基于大语言模型（LLM）的医疗保健聊天机器人。考虑到用户类型、领域类型和任务类型等混杂变量，指标分为准确性、可信度、同理心和性能。每个指标都解决了特定的医疗保健相关问题，并详细介绍了它们的获取和评估方法。
### 评估医疗保健聊天机器人的挑战
使用所提出的指标评估医疗保健聊天机器人存在挑战。指标之间存在类别内和类别间的关联。评估方法（自动或基于人工）的选择会影响指标得分。提示工程和模型参数修改在评估过程中也起着重要作用。
### 迈向一个有效的评估框架
一个有效的医疗保健聊天机器人评估框架应该是协作式的、端到端的且标准化的。其组成部分包括模型、可配置环境（混杂变量、提示技术和参数、评估方法）、评估工具、接口以及交互用户。该框架旨在使指标分数解释标准化，以便进行更好的模型比较。
### 结论
生成式人工智能聊天机器人在医疗保健领域具有巨大潜力。本文介绍了分为四类的定制评估指标。混杂变量会影响指标定义和评估。本文研究了开发和实施这些指标时面临的挑战。未来的工作包括实施评估框架，并通过案例研究建立统一的基准。
...
个性化文本生成提供了一种专门机制，用于传递特定于用户个人情境的内容。尽管该领域的研究进展迅速，但评估仍然是一项挑战。诸如BLEU和ROUGE等传统自动化指标主要衡量与人工撰写参考文本的词汇相似度，无法将个性化与其他细微的语义方面区分开来，因此难以捕捉个性化生成内容质量的细微差别。另一方面，人工判断的获取成本很高，尤其是在个性化评估领域。受这些挑战的启发，我们探索使用大语言模型（LLMs）来评估个性化文本生成，并考察它们理解细微用户情境的能力。我们提出了AuPEL，这是一种新颖的评估方法，它提炼出生成文本的三个主要语义方面：个性化、质量和相关性，并自动衡量这些方面。为了验证AuPEL的有效性，我们精心设计了对照实验，比较了大语言模型做出的评估判断与人工注释者做出的判断的准确性，并对所提出指标的一致性和敏感性进行了严格分析。我们发现，与现有评估指标相比，AuPEL不仅能更准确地根据模型的个性化能力对其进行区分和排名，而且在这项任务中还表现出值得称赞的一致性和效率。我们的工作表明，使用大语言模型作为个性化文本生成的评估工具优于传统的文本相似度指标，尽管仍然存在一些有趣的新挑战。
