...
def compute_scores(df, lam=0.2, group_by="task"):
    df["quality"] = [f1_score(p, r) for p, r in zip(df["answer"], df["reference"])]
    results = []
...
def visualize(df, agg, outdir):
    ...
    plt.title("各模型平均问答质量")
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "qa_quality_avg_bar.png"))
    plt.close()
    plt.figure(figsize=(9, 5))
    sns.barplot(data=agg, x="model", y="fair_quality_score", hue=None)
    plt.ylabel("公平化质量分数")
    plt.title("各模型公平化综合质量")
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "qa_quality_fair_bar.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "qa_quality_box.png"))
    plt.close()
    ...
    plt.xlabel("模型")
    plt.ylabel("问题ID")
    plt.title("问题-模型质量热力图")
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "qa_quality_heatmap.png"))
    plt.close()
...
def main():
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "qa_quality_scores.csv")
    df.to_csv(csv_path, index=False, encoding="utf-8")
    print("saved", csv_path)
    print("saved", os.path.join(outdir, "qa_quality_avg_bar.png"))
    print("saved", os.path.join(outdir, "qa_quality_fair_bar.png"))
    print("saved", os.path.join(outdir, "qa_quality_box.png"))
    print("saved", os.path.join(outdir, "qa_quality_heatmap.png"))
    print("done")
